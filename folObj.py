...
class BinaryLogicalConnector(FoLOperator):
    def __init__(self, name: str, f1: Union[Formula, Concept, TBoxQuantifier], f2: Union[Formula, Concept, TBoxQuantifier]):
        super().__init__(name)
        self.formula1 = f1
        self.formula2 = f2
...
class And(BinaryLogicalConnector):
    def __init__(self, f1: Union[Formula, Concept, TBoxQuantifier], f2: Union[Formula, Concept, TBoxQuantifier]):
...
class Not(FoLOperator):
    def __init__(self, formula: Union[Formula, Concept]):
        super().__init__("Not")
        self.formula = formula
...
class ContainingVariable:
    def __init__(self):
        pass

    def __repr__(self):
        return "ContainingVariable"


class Concept(object):
    def __init__(self, name: str, definition: Union[PredicateSymbol, Formula]):
        self.name = name
        self.definition = definition

    def __repr__(self):
        return f"Concept({self.name!r}:{self.definition!r})"



class PrimitiveConcept(PredicateSymbol, Concept):
    def __init__(self, name: str):
        PredicateSymbol.__init__(self, name, 1)
        Concept.__init__(self, name, self)

    def __repr__(self):
        return f"PC({self.name!r})"

    def __call__(self, param: Term) -> ConceptAssertion:
        return ConceptAssertion(self, param)



class DefinedConcept(Concept):
    def __init__(self, name: str, definition: Union[Formula, TBoxQuantifier]):
        Concept.__init__(self, name, definition)
        self.name = name
        self.definition = definition

    def __repr__(self):
        return f"DC({self.name!r}: {self.definition!r})"

    def __call__(self, param: Term) -> ConceptAssertion:
        return ConceptAssertion(self, param)


TBox = List[Union[DefinedConcept, PrimitiveConcept]]


class Relation(PredicateSymbol, ContainingVariable):
    def __init__(self, name: str):
        super(Relation, self).__init__(name, 2)

    def __repr__(self):
        return f"R({self.name!r})"

    def __call__(self, param1: Term, param2: Term) -> RelationAssertion:
        return RelationAssertion(self, [param1, param2])


class TBoxQuantifier(object):
    def __init__(self, name: str, relation: Relation, concept: Union[Concept, DCConstant, TBoxQuantifier, And, Or, Not]):
        self.name = name
        self.relation = relation
        self.concept = concept

    def __repr__(self):
        return f"TBoxQuantifier({self.name!r}, {self.relation!r}, {self.concept!r})"


class TBoxForAll(TBoxQuantifier):
    def __init__(self, relation: Relation, concept: Union[Concept, DCConstant, TBoxQuantifier, And, Or, Not]):
        super(TBoxForAll, self).__init__("ForAll", relation, concept)

    def __repr__(self):
        return f"ForAll ({self.relation!r}.{self.concept!r})"


class TBoxExists(TBoxQuantifier):
    def __init__(self, relation: Relation, concept: Union[Concept, DCConstant, TBoxQuantifier, And, Or, Not]):
        super(TBoxExists, self).__init__("Exists", relation, concept)
...
class Assertion:
    def __init__(self, predicate: Union[Concept, Relation], param: List[Term]):
        self.predicate = predicate
        self.param = param

    def __repr__(self):
        return f"Assertion({self.predicate!r} {self.param!r})"


class ConceptAssertion(Assertion):
    def __init__(self, concept: Concept, param: Term):
        super(ConceptAssertion, self).__init__(concept, [param])
        self.concept = concept
        self.param: Term = param

    def __repr__(self):
        return f"ConceptAssertion({self.concept!r} {self.param!r})"


class RelationAssertion(Assertion):
    def __init__(self, relation: Relation, param: List[Term]):
        super(RelationAssertion, self).__init__(relation, param)
        self.relation = relation
        self.param = param
        if len(param) != 2:
            raise ValueError("incorrect param number")

    def __repr__(self):
        return f"RelationAssertion({self.relation!r} {self.param!r})"
...
class DCConstant(object):
    ...
    def __repr__(self):
        return f"DCConstant({self.name!r})"


class TTop(DCConstant):
    def __init__(self):
